...
// Search box search for user info php
    if(isset($_GET['searchBtn'])){ 
       ...
       $db->query("SELECT * FROM lost_items WHERE item_name LIKE '%$c_search%'");
  ...
  <div class="media">
    ...
    $itemimg = $show['image'];
      ...
      <div>
      <?php
      echo"<small>{$show['owner_name']}, {$show['date']}</small>"?>
      </div>
    </div>
    ...
    <?php
      ...
      <div>
      <?php
      echo"<strong>Lost At: </strong><a>{$show['lost_at']}</a>"?>
      </div>
      <div>
      <?php
       echo "<strong>Owner Contact: </strong><a> {$show['owner_contact']}</a>
       <strong>Owner Msg: </strong><a> {$show['owner_msg']}</a>";?>
    </div>
  </div>
  <br>
</div>
